...
/**
 * ダッシュボードHTMLを生成する。
 * @param reports - CSP違反とネットワークリクエストの配列
 * @param lastUpdated - 最終更新日時（ISO形式）
 * @returns ダッシュボードHTML文字列
 */
...
<body>
  <div class="container">
    ...
    <div class="card">
      ${
        violations.length === 0
          ? '<p class="empty">No CSP violations recorded</p>'
          : `
      <table>
        <thead>
          <tr>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
      <div class="card">
        <h3>By Directive</h3>
        ${
          Object.keys(directiveStats).length === 0
            ? '<p class="empty">No data</p>'
            : `
        <table>
          <thead><tr><th>Directive</th><th>Count</th></tr></thead>
          <tbody>
            ${Object.entries(directiveStats)
              .sort((a, b) => b[1] - a[1])
              .map(([d, c]) => `<tr><td><code>${d}</code></td><td>${c}</td></tr>`)
              .join("")}
          </tbody>
        </table>
        `
      ...
      <div class="card">
        ...
        <table>
          ...
          <tbody>
            ${Object.entries(initiatorStats)
              .sort((a, b) => b[1] - a[1])
              .map(([i, c]) => `<tr><td><span class="badge">${i}</span></td><td>${c}</td></tr>`)
              .join("")}
          </tbody>
        </table>
        `
