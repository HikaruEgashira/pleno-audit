...
import type { Period, TabType, TotalCounts } from "./dashboard-types";
import { getPeriodMs, getStatusBadge } from "./dashboard-utils";
...
export function useDashboardData({ period, setActiveTab }: DashboardDataOptions) {
  ...
  const { notifications, addNotification, dismissNotification } =
    useNotifications();
  const [lastNotifiedEventId, setLastNotifiedEventId] = useState<string | null>(
    null
  );
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const [
        ...
        safeMessage(
          {
            ...
            data: { type: "csp-violation", since: sinceISO, limit: 500 },
          },
          { reports: [], total: 0 }
        ),
        safeMessage(
        ...
        safeMessage(
          { type: "GET_EVENTS", data: { since: sinceTs, limit: 500 } },
          { events: [], total: 0 }
        ),
        safeMessage({ type: "GET_EVENTS_COUNT", data: { since: sinceTs } }, {
          count: 0,
        }),
      ...
      if (configResult) {
        setConnectionMode(configResult.mode as "local" | "remote");
      }
      ...
      if (aiPromptsCountResult) {
        setTotalCounts((prev) => ({
          ...prev,
          aiPrompts: aiPromptsCountResult.count ?? 0,
        }));
      }
  ...
  }, [events, loading, lastNotifiedEventId, addNotification, setActiveTab]);
  ...
  const directiveStats = useMemo(() => {
    ...
    for (const v of violations) {
      stats[v.directive || "unknown"] = (stats[v.directive || "unknown"] ?? 0) + 1;
    }
  ...
  const nrdServices = useMemo(
    () => services.filter((s) => s.nrdResult?.isNRD),
    [services]
  );

  const loginServices = useMemo(
    () => services.filter((s) => s.hasLoginPage),
    [services]
  );

  const status = useMemo(
    () => getStatusBadge(nrdServices.length, violations.length, aiPrompts.length),
    [nrdServices.length, violations.length, aiPrompts.length]
  );
...
export function useDashboardFilters({
  ...
  const filteredNetworkRequests = useMemo(() => {
    ...
    return networkRequests.filter(
      (r) => r.url.toLowerCase().includes(q) || r.domain.toLowerCase().includes(q)
    );
