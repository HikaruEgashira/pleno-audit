...
interface DomainRiskServiceDependencies {
  logger: LoggerLike;
  getStorage: () => Promise<{
    nrdConfig?: NRDConfig;
  ...
  updateService: (domain: string, updates: Partial<DetectedService>) => Promise<void>;
  addEvent: (event: {
    type: "nrd_detected" | "typosquat_detected";
    domain: string;
    timestamp: number;
    details: NRDDetectedDetails | TyposquatDetectedDetails;
  }) => Promise<unknown>;
  getAlertManager: () => {
    alertNRD: (params: {
      domain: string;
      ...
      confidence: NRDResult["confidence"];
    }) => Promise<void>;
    alertTyposquat: (params: {
      domain: string;
...
export interface DomainRiskService {
  ...
  handleTyposquatCheck: (domain: string) => Promise<TyposquatResult | { skipped: true; reason: string }>;
}

export function createDomainRiskService(deps: DomainRiskServiceDependencies): DomainRiskService {
  const nrdCache = new Map<string, NRDResult>();
  const typosquatCache = new Map<string, TyposquatResult>();
  ...
  function checkTyposquat(domain: string): TyposquatResult {
    if (!typosquatDetector) {
      typosquatDetector = createTyposquatDetector(DEFAULT_TYPOSQUAT_CONFIG, typosquatCacheAdapter);
    }
