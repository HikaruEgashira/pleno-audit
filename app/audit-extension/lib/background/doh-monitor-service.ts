...
interface DoHMonitorServiceDeps {
  logger: LoggerLike;
...
export function createDoHMonitorService(deps: DoHMonitorServiceDeps) {
  ...
  function attachMonitor(nextMonitor: DoHMonitor) {
    monitor = nextMonitor;

    monitor.onRequest(async (record: DoHRequestRecord) => {
      try {
        const storage = await deps.getStorage();
